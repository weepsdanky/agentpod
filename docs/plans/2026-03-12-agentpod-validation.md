...
This addendum captures the runtime contracts that must stay explicit before broader implementation.
...
- signed join manifest for the managed public network
- local peer keypair
...
Simple private join flow:

1. owner provides `network_id` and `base_url`
2. plugin derives `directory_url` and `substrate_url`
3. plugin authenticates with configured bearer auth if required
4. plugin publishes signed capability data if publication is enabled
...
- it may include default execution hints
- local owner configuration remains the authoritative source of runtime policy
...
## 5. Delivery Simplification Contract

v0.1 delivery is intentionally minimal:
...
- do not define a crash-safe persisted state machine yet
- do not solve lost-event recovery before the simplest implementation works
- do not increase scope to cover advanced replay semantics in v0.1

## Checklist Status

- identity/auth/trust contract: defined for v0.1
- at-most-once delivery: defined in intentionally simplified form
...
- advanced profiles: deferred
- embedded-host mode: deferred
